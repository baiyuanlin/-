...
class Details extends React.Component {
	...
	componentDidMount(){
		...
		$.ajax({
			type: "get",
			url: "http://www.baidu.com/api/fangyuan",
			async: true,
			dataType: "json",
		...
		$.ajax({
			type: "get",
			url: "http://www.baidu.com/api/tuijianfangyuan",
			async: true,
			dataType: "json",
			success: function (data) {
	...
	componentDidUpdate(){
		...
		$('.openmap').click(function(){
			...
			$('#map_warp').show()
			var x = $('.openmap').attr("data-x")
			var y = $('.openmap').attr("data-y")
			var add = $('.openmap').attr("data-add")
			console.log(x,y,add)
			_this.setState({
				x:y,
				y:y,
	...
	render(){
		return(
			<div className = "Details_warp"> 
				...
				<div className="Deatails_main_warp">
				<div className="Deatails_main">
					<div className="Deatails_main_article">
						...
						{this.state.arr.map(function(item,i){
							...
							return(
						<div className="Deatails_main_house" key={i}>
							...
							<div className="Deatails_main_house_detail" >
								<div className="house_tit_warp">						
										<a href="javascript:void(0);" target="_blank" className="house_tit">{item.title}</a>
                            	</div>
								...
								<div className="house_introduction">
									<p>
										<span>{item.room} </span>| 
										<span> 整租 </span>| 
										<span> 宜住{item.renshu}人 </span>|
										<span> {item.pl}条评论</span>  
									</p>	                           
									...
									<div className="house_price">
										<span>￥{item.price}</span>/晚
									</div>
									<div className="near_reserve">
										近期预订<span>{item.yd} 晚</span>
									</div>
									<div className="hr"></div>
                            	</div>
								<div className="house_introduction_xia">
									<p>
										<span className="shang">{item.pl}</span>
										<span className="xia">
											<b className="pls"><Icon type="message" /></b>评论数
										</span>
									</p>
									<p>
										<span className="shang">{item.tjs}</span>
										<span className="xia">
									...
									<p className="fy_map">
										<span  className="shang openmap"data-x={item.x} data-y={item.y} data-add={item.address}><Icon type="environment" /></span>
						...
						<div id="map_warp">
							<div className="Deatail_main_house_map">
								<p ><span id="closemap" >关闭</span></p>
								<Map center={{ lng: this.state.x, lat: this.state.y }} zoom="12" style={{ "height": "400px" }}>
									<Marker position={{ lng: this.state.x, lat: this.state.y }} />
									<NavigationControl />
									<InfoWindow position={{ lng:this.state.x, lat: this.state.y }} text="上海" title={this.state.add} />
								</Map>
					...
					<div className="Deatails_main_sidebar">
						...
						<div className="tjfy_warp">
							<div className="main_sidebar_list_tjfy">
								<p><div></div>推荐房源</p>
						{
							this.state.tjfy.map(function(item,i){
								return(

								<div className="tjfy_warp" key={i}>
									<div className="tjfy_img">
										<img src={require("./Deatails_imgs/tjfy1.jpg")}/ >
									</div>
									
									<div className="reco_house_price_warp">
